...
def obtener_chofer_mas_cercano(lat_cli, lon_cli, tipo_sol):
    ...
    if df_c.empty or df_u.empty: return None, None, None, "N/A"
    ...
    if libres.empty: return None, None, None, "N/A"
    ...
    if mejor is not None:
        ...
        prefijos = {"Ecuador": "593", "Colombia": "57", "Perú": "51"}
        cod = prefijos.get(pais_chofer, "593")
        
        if pais_chofer == "Ecuador" and t.startswith("09"): t = cod + t[1:]
        elif not t.startswith(cod): t = cod + t
        
        placa = str(mejor.get('Placa', 'S/P')) # Obtener placa o S/P si no existe
...
if st.session_state.viaje_confirmado:
    ...
    try:
        ...
        if dp['foto'] and "http" in dp['foto']:
            ...
            if id_f: st.markdown(f'<div style="text-align:center; margin-bottom:15px;"><img src="https://lh3.googleusercontent.com/u/0/d/{id_f}" style="width:120px;height:130px;border-radius:50%;object-fit:cover;border:4px solid #25D366;"></div>', unsafe_allow_html=True)
